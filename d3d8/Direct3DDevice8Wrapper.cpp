...
Direct3DDevice8Wrapper::Direct3DDevice8Wrapper(IDirect3DDevice8 ** pDirect3DDevice8, D3DPRESENT_PARAMETERS * pPresentationParameters, int AnisotropicLevel)
{
	Direct3DDevice8 = *pDirect3DDevice8;
	...
	PresentationParameters = new D3DPRESENT_PARAMETERS();
	*PresentationParameters = *pPresentationParameters;
	this->AnisotropicSamples = AnisotropicLevel;
}
...
HRESULT Direct3DDevice8Wrapper::SetTextureStageState(DWORD Stage, D3DTEXTURESTAGESTATETYPE Type, DWORD Value) {
	if (AnisotropicSamples != 0)
	{
		if (AnisotropicSamples > 16)
